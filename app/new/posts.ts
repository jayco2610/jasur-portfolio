...
export type CardPost = {
  ...
  /* Ключ рубрики: то, по чему страница Log фильтрует список. Держим рядом
     ...
     считать ключ обратно из русского названия, они разойдутся при первом
     переименовании рубрики. */
  rubricKey: string;
  title: string;
  /* Нужно только крупной первой статье на Log. В карточках описаний нет
     намеренно: они разной длины и ломают высоту соседних карточек. */
  description: string;
  /* Флаг «главное» из файла статьи. Сортировку списка он не меняет (см.
     ниже), но Log выбирает по нему, какая статья встанет крупной первой:
     ровно для этого флаг и заведён. */
  featured: boolean;
};
